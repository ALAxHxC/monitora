...
module.exports.deleteUserByid = async function(id,res){
	try{
		let result = await userController.deleteDocumentById(id);
		res.status(200).send(result)
	}catch(err){
		res.status(400).send({error: errors.noDeleteUser,cause:err.message});
	}
}
